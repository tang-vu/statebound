...
const workflow=JSON.parse(readFileSync('submission/binance-workflow.json','utf8'));
assert.ok(markup.includes('id="binance"'));
assert.ok(markup.includes(workflow.payload.mandate.goal));
assert.ok(markup.includes(workflow.payload.provenance.observedAt));
assert.deepEqual(await fetch(`${base}/binance-workflow.json`).then(r=>r.json()),workflow);
assert.deepEqual(await fetch(`${base}/binance-read.json`).then(r=>r.json()),JSON.parse(readFileSync('examples/binance-read.json','utf8')));
...
try {
  for(const viewport of [{width:1440,height:900},{width:390,height:844}]){
    ...
    assert.equal(await page.locator('.bridge-flow article').count(),3);
    await page.locator('#binance summary').click();
    assert.ok(await page.locator('#binance').innerText().then(text=>text.includes(workflow.manifest)));
    await page.locator('#binance summary').click();
