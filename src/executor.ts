import { type Mandate,type Plan,type State,type Effect,type Observation,initial,transition,observe,validate,D,SCALE } from './core.js';
...
export type Checkpoint={binding:string;state:State;pending:string|null;stopped:boolean;deadline?:number};
function ingest(m:Mandate,state:State,effect:Effect,observation:Observation|null) {
  ...
  if(observation.kind==='order'&&D(observation.net??'0')*D(m.limitPrice)>D(observation.debit??'0')*SCALE) {state.k.invalid=true;state.reason='Quantity and debit contradict the fixed price model';return;}
...
export async function execute(m:Mandate,p:Plan,ledger:Ledger,run:string,adapter:RuntimeAdapter,approve:(effect:Effect)=>Promise<Approval>,stopped:()=>boolean=()=>false,onEvent:(state:State,effect:Effect,observation:Observation|null)=>void=()=>{}) {
  ...
  ledger.lease(m.account,run);let state=prior?.state??initial(p);let halted=prior?.stopped??false;const deadline=prior?.deadline??Date.now()+p.maxDurationMs;
  try {
    ...
    if(prior?.pending) {
      ...
      ledger.checkpoint(run,{binding:binding(m,p),state,pending:null,stopped:halted,deadline});
    }
    while(state.status==='RUNNING') {
      halted=halted||stopped()||Date.now()>=deadline||Date.now()>=Date.parse(m.expiresAt);
      ...
      if(t.effect.kind==='submit') {
        ...
        if(stopped()||Date.now()>=deadline) {state.status='UNRESOLVED';state.reason='Stopped or duration expired before dispatch';break;}
        if(!ledger.prepare(m,p,run,t.effect,approval,state,Date.now(),deadline)) {state.status='UNRESOLVED';state.reason='Existing attempt tombstone prevents reuse';break;}
      }
      state=t.state;
      ...
      ledger.reconcile(m.account,state);
      ledger.transaction(()=>{ledger.checkpoint(run,{binding:binding(m,p),state,pending:null,stopped:halted,deadline});ledger.event(run,{state,effect:t.effect,observation});});
      onEvent(state,t.effect,observation);
    }
    ledger.checkpoint(run,{binding:binding(m,p),state,pending:null,stopped:halted,deadline});return state;
