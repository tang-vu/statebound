...
| `npm run mandate:compile -- "buy around 15 USDT of BNB, maximum total debit 20 USDT"` | Derive a reviewable synthetic mandate using a narrow documented grammar |
...
Install the optional test browser with `npx playwright install chromium`. The application itself does not need Playwright at runtime. Recording additionally requires ffmpeg/ffprobe and the accepted MiMo audio files described in [narration reproduction](docs/MIMO_NARRATION.md). See [recording instructions](submission/SUBMISSION_DRAFT.md).
...
- **Demo:** [actual recorded application video with MiMo Dean narration](submission/demo.mp4), nine chapters, timed captions and a recorded Codex candidate. Voice and playback remain at original speed. Each voice clip is checked with MiMo ASR; see [narration provenance](docs/MIMO_NARRATION.md). [Exported evidence](submission/demo-evidence.json) can be verified with the CLI.
